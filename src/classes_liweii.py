...
class ColorFilter:
    def __init__(self):
        ...
        self.pub = rospy.Publisher('camera/colorfilter/test', Image, queue_size=2)
    ...
    def roi_red(self,image):
        #add mask for roi
        imgHSV = cv2.cvtColor(image,cv2.COLOR_BGR2HSV)
        lower_red = np.array([0,134,0])  #-- Lower range --
        upper_red = np.array([179,255,255])  #-- Upper range --

        red_mask = cv2.inRange(imgHSV, lower_red, upper_red)
        image00 = self.bridge.cv2_to_imgmsg(red_mask)
        self.pub.publish(image00)

        (_,cont, _) = cv2.findContours(red_mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        # print 'lne',len(cont)

        if len(cont)==1:
            perim = cv2.arcLength(cont[0], True)
            approx = cv2.approxPolyDP(cont[0], .1 * perim, True)
            polygons = np.array(approx)
    ...
    def paper_filter(self,image):
        ...
        red_mask = self.roi_red(image0)
        image = cv2.bitwise_and(image,red_mask)
        imgHSV = cv2.cvtColor(image,cv2.COLOR_BGR2HSV)
    ...
    def paper_filter_temp(self,image):
        #return paper mask
        imgHSV = cv2.cvtColor(image,cv2.COLOR_BGR2HSV)
        imgG = cv2.cvtColor(imgHSV,cv2.COLOR_BGR2GRAY)
        cv2.imshow('imgg',imgG)
        ...
        imgC = cv2.morphologyEx(img0,cv2.MORPH_CLOSE,(11,11))
        imgC = cv2.GaussianBlur(imgC,(9,9),0)
        (_,imgC) = cv2.threshold(imgC,180,255,cv2.THRESH_BINARY | cv2.THRESH_OTSU)
        # cv2.imshow('imgc',imgC)
        imgC = cv2.bitwise_not(imgC)
        # cv2.imshow('imgc',imgC)
        cm = CornerMatch_new()
...
class GetTrans_new:
    ...
    def detect_new(self, frame, side_view=0):
        ...
        for c in cont:
            ...
            print 'len approax',len(approx),approx
        ...
        if best_approx is not None:
            ...
            if len(pts_dst) < 4: #at least 4 points are needed (not co-linear points)
                ...
                M1 = cv2.getAffineTransform(np.float32(pts2),np.float32(pts1))
                frame0 = copy.deepcopy(frame)
                frame0 = cv2.warpAffine(frame0,M,(290,290))
                cv2.circle(frame0, (int(pts_src[0][0]), int(pts_src[0][1])), 5, (0, 0, 255), 2)
                cv2.circle(frame0, (int(pts_src[1][0]), int(pts_src[1][1])), 5, (0, 0, 255), 2)
                cv2.circle(frame0, (int(pts_src[2][0]), int(pts_src[2][1])), 5, (0, 0, 255), 2)
                # cv2.circle(frame0, (0,145), 5, (0, 0, 255), 2)
                # cv2.circle(frame0, (145,0), 5, (0, 0, 255), 2)
                cv2.imshow('frame0',frame0)
                new_dst_point = (int((pts_dst[0][0]+pts_dst[1][0]+pts_dst[2][0])/3),int((pts_dst[0][1]+pts_dst[1][1]+pts_dst[2][1])/3),1)
                new_dst_point = np.dot(M1,new_dst_point)
            ...
            cv2.circle(frame, (int(center1[0]), int(center1[1])), 5, (0, 0, 255), 2)
            center2 = np.dot(h,(145,0,1))
            # print 'center2',center2
            cv2.circle(frame, (int(center2[0]), int(center2[1])), 5, (0, 255, 0), 2)
            center3 = np.dot(h,(0,145,1))
            # print 'center3',center3
            cv2.circle(frame, (int(center3[0]), int(center3[1])), 5, (0, 255, 255), 2)
            print 'status',status

            c4 = np.dot(h,(290,0,1))
            c5 = np.dot(h,(0,290,1))
            cv2.circle(frame, (int(c4[0]), int(c4[1])), 5, (0, 255, 255), 2)
            cv2.circle(frame, (int(c5[0]), int(c5[1])), 5, (0, 255, 255), 2)
    ...
    def detect(self, frame, ori_img, side_view=0):
        ...
        # (_,cont, _) = cv2.findContours(imgC.copy(), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
        # frame = cv2.drawContours(frame,cont,-1,(0,0,255),3)
        # ori_img = cv2.drawContours(ori_img,cont,-1,(0,0,255),3)
        # print 'cont num',len(cont)
        best_approx = None
        lowest_error = float("inf")
        ...
        for c in cont:
            ...
            if len(approx) == len(self.pts_src):
                right, error, new_approx = su.rightA(approx, 50,side_view) #80#change the thresh if not look vertically
                # print(right)
                new_approx = np.array(new_approx)
                print 'new approx',new_approx
                if error < lowest_error and right:
        ...
        if best_approx is not None:
            cv2.drawContours(frame, [best_approx], 0, (255, 0, 0), 3)
            cv2.drawContours(ori_img, [best_approx], 0, (255, 0, 0), 3)
            print 'len approx',len(best_approx)

            for i in range(0, len(best_approx)):
                pts_dst.append((best_approx[i][0][0], best_approx[i][0][1]))
                if pts_dst[-1][0]<105 or pts_dst[-1][1]<75:
                    cv2.circle(frame, pts_dst[-1], 3, (i*30, 0, 255-i*20), 3)
                    # cv2.circle(ori_img, pts_dst[-1], 10, (i*30, 0, 255-i*20), 3)
                    cv2.circle(ori_img, pts_dst[-1], 10, (0, 255, 0), 3)
                    print 'pts',pts_dst[-1]
            ...
            # print 'center1',center1
            cv2.circle(frame, (int(center1[0]), int(center1[1])), 5, (0, 0, 255), 2)
            cv2.circle(ori_img, (int(center1[0]), int(center1[1])), 5, (0, 0, 255), 2)
            cv2.imshow('ori0',ori_img)
            # center2 = np.dot(h,(145,0,1))
            # print 'center2',center2
            ...
            zR = np.matrix([[math.cos(Rot[2]), -math.sin(Rot[2])], [math.sin(Rot[2]), math.cos(Rot[2])]])
            cv2.putText(ori_img, 'rX: {:0.2f} rY: {:0.2f} rZ: {:0.2f}'.format(Rot[0] * 180 / np.pi, Rot[1] * 180 / np.pi, Rot[2] * 180 / np.pi), (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255))
            # cv2.putText(ori_img, 'tX: {:0.2f} tY: {:0.2f} tZ: {:0.2f}'.format(Translation[0][0], Translation[1][0], Translation[2][0]), (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255))
            cv2.putText(ori_img, 'tX: {:0.2f} tY: {:0.2f} tZ: {:0.2f}'.format(Translation[0], Translation[1], Translation[2]), (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255))
